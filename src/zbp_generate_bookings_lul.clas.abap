CLASS zbp_generate_bookings_lul DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_lul IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_lul.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_lul.
*   insert the new table entries
    INSERT ztbooking_lul FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_lul INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
