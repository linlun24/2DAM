...
* 1) define the data buffer
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_buffer.
             INCLUDE TYPE   ztbooking_lul AS data.
...
CLASS lsc_zi_booking_lul DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_zi_booking_lul IMPLEMENTATION.

  METHOD save.
    DATA lt_data TYPE STANDARD TABLE OF ztbooking_lul.

    lt_data = VALUE #(  FOR row IN lhc_buffer=>mt_buffer WHERE  ( flag = 'C' ) (  row-data ) ).
    IF lt_data IS NOT INITIAL.
      INSERT ztbooking_lul FROM TABLE @lt_data.
    ENDIF.
    ...
    IF lt_data IS NOT INITIAL.
      DELETE ztbooking_lul FROM TABLE @lt_data.
    ENDIF.
    ...
    IF lt_data IS NOT INITIAL.
      UPDATE ztbooking_lul FROM TABLE @lt_data.
    ENDIF.
...
CLASS lhc_booking IMPLEMENTATION.
  ...
  METHOD create.
    ...
    IF entities IS NOT INITIAL.
      SELECT SINGLE MAX( booking ) FROM ztbooking_lul INTO @DATA(lv_max_booking).
    ENDIF.
  ...
  METHOD update.
    ...
    LOOP AT entities INTO DATA(ls_update).
      ...
      IF sy-subrc <> 0.
        ...
        SELECT SINGLE * FROM ztbooking_lul WHERE booking = @ls_update-booking INTO @DATA(ls_db).
  ...
  METHOD read.
    ...
    LOOP AT keys INTO DATA(ls_booking_key).
      ...
      ELSE.
        SELECT SINGLE * FROM ztbooking_lul WHERE booking = @ls_booking_key-booking INTO @DATA(ls_db).
        IF sy-subrc = 0.
